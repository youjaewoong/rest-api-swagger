...
public class StringUtil {
	 ...
	 * @return	String	변환문자
	 */
	public static String getMaskTimeMin(String str, String mask){
		StringBuffer strBuf = new StringBuffer("");
		str = str.trim();
		if (str.equals("") || str.equals("0") || str.length() != 4){
			return str;
...
//	 * @return
//	 */
//	public static String makeIfId(ApprovalLine approvalLine) {
//		
//		String lsReturn = "H"+approvalLine.getSystemCode()+approvalLine.getSender()+approvalLine.getBiz_id()+CurrentDateTime.getDate().substring(2, 8)+CurrentDateTime.getTime()+(int)(java.lang.Math.random()*10);
//		return lsReturn;
//	}
//	
//	/**
...
//	 * @return
//	 */
//	public static String makeIfIdByType(ApprovalLine approvalLine) {
//		String lsReturn = "H"+approvalLine.getSystemCode()+approvalLine.getSender()+approvalLine.getBiz_id()+CurrentDateTime.getDate().substring(3, 8)+CurrentDateTime.getTime()+(int)(java.lang.Math.random()*10)+approvalLine.getType();
//		return lsReturn;
//	}
//	
	/**
	...
	public static String replaceStr(String str){
		...
		if(check){
			...
			rtn = rtn.replaceAll("</script>", " ");
			rtn = rtn.replaceAll("select", " ");
			rtn = rtn.replaceAll("trim", " ");
			rtn = rtn.replaceAll("and", " ");
			rtn = rtn.replaceAll("alert", " ");
			rtn = rtn.replaceAll("script", " ");
		}
	...
	public static String replaceStrScrpt(String str){
		boolean check = true;
		String rtn = "";
		if(str.equals("")) check = false;
		...
		while(check){
			...
			if(rtn.indexOf("select") > -1){
				rtn = rtn.replaceAll("select", "");
				check = true;
			}
			if(rtn.indexOf("trim") > -1){
				rtn = rtn.replaceAll("trim", "");
				check = true;
			}
			if(rtn.indexOf("and") > -1){
				rtn = rtn.replaceAll("and", "");
				check = true;
			}
			if(rtn.indexOf("alert") > -1){
				rtn = rtn.replaceAll("alert", "");
				check = true;
			}
			if(rtn.indexOf("script") > -1){
			...
			if(rtn.indexOf("xml") > -1){
				rtn = rtn.replaceAll("xml", "");
				check = true;
			}
			if(rtn.indexOf("WEB-INF") > -1){
				rtn = rtn.replaceAll("WEB-INF", "");
				check = true;
			}
		}
		return rtn;
	...
	/**
	 * 문자열에서 코볼특수문자를 제거후 대문자로 변환 후 문자열 리턴
	...
	public static String fileExtention(String fileName) {
	 ...
	 //파일 추가시 따옴표 제거
	 public static String replaceStrFile(String str){
			boolean check = true;
			if(str.equals("")) check = false;
